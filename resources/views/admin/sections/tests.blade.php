...
<!-- Test Details Modal -->
<div id="test-details-modal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-lg bg-white">
        <div class="flex items-center justify-between p-4 border-b">
            <h3 class="text-lg font-semibold text-gray-900">Détails du Test</h3>
            <button onclick="closeTestDetailsModal()" class="text-gray-400 hover:text-gray-600">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                </svg>
            </button>
        </div>
        <div id="test-details-content" class="p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="space-y-2 text-sm">
                        <p><span class="font-medium">Nom:</span> ${test.user.name} ${test.user.last_name}</p>
                        <p><span class="font-medium">ID:</span> ${test.user.identification}</p>
                        <p><span class="font-medium">Formateur:</span> ${test.formateur.name} ${test.formateur.last_name}</p>
                    </div>
                </div>
                <div>
                    <h4 class="font-semibold text-gray-900 mb-2">Détails du Test</h4>
                    <div class="space-y-2 text-sm">
                        <p><span class="font-medium">Catégorie:</span> ${test.category_name}</p>
                        <p><span class="font-medium">Score:</span> ${test.pourcentage}%</p>
                        <p><span class="font-medium">Date:</span> ${formatDate(test.created_at)}</p>
                        <p><span class="font-medium">Statut:</span> ${test.is_retake ? 'Test repris' : 'Premier test'}</p>
                    </div>
                </div>
            </div>
        </div>
